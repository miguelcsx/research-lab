...
def _invoke(project: Any, kind: str, name: str, ctx: RuntimeContext) -> Any:
    ...
    if isinstance(target_ref, str) and target_ref:
        return callable_obj(_resolve_component(project, target_ref), ctx)
...
def _invoke_dataset(project: Any, name: str, ctx: RuntimeContext) -> dict[str, Any]:
    ...
    ctx.report_progress("dataset", name, "running", 0, None, "")
    result = cast(
        dict[str, Any],
        json.loads(
            execute_dataset(
    ...
    ctx.report_progress("dataset", name, "completed", result.get("records", 0), None, "")
    return result
...
def _dataset_source(project: Any, name: str, metadata: Mapping[str, Any]) -> Any:
    try:
        ...
        return _resolve_component(project, _component_ref(metadata.get("source"), "source"))
...
def _dataset_stages(
    ...
    ref = _component_ref(metadata.get("pipeline"), "pipeline")
...
def _dataset_stage(project: Any, value: object) -> tuple[str, Any, dict[str, Any]]:
    ref = _component_ref(value, "transform")
    kind, name = ref.split(":", 1)
    config = (
        {str(key): child for key, child in value.items() if key != "ref"}
        if isinstance(value, Mapping)
        else {}
    )
    return ref, project.resolve(kind, name), config
...
def _dataset_sinks(project: Any, name: str, metadata: Mapping[str, Any]) -> list[Any]:
    ...
    if not isinstance(sinks, list):
        sinks = [sinks]
    return [_resolve_component(project, _component_ref(sink, "sink")) for sink in sinks]


def _resolve_component(project: Any, reference: str) -> Any:
    ...
    if ":" in rest:
        ...
        return load(path)
    return _materialize(project.resolve(kind, rest))
...
def _component_ref(value: object, default_kind: str) -> str:
