    ...
    pub fn validate(&self) -> RlabResult<()> {
        if self.schema_version != 1 {
            return Err(RlabError::Validation {
                message: "unsupported workflow schema_version".to_string(),
            });
        }
...
impl WorkflowStep {
    pub fn validate(&self) -> RlabResult<()> {
        if self.schema_version != 1 {
            return Err(RlabError::Validation {
                message: "unsupported workflow step schema_version".to_string(),
            });
        }
        if self.name.trim().is_empty() {
            return Err(RlabError::Validation {
        ...
        match &self.kind {
            WorkflowStepKind::Python { module, qualname } => {
                if module.trim().is_empty() || qualname.trim().is_empty() {
                    return Err(RlabError::Validation {
                        message: "python workflow step requires module and qualname".to_string(),
                    });
                }
            }
...
impl ExternalStep {
    pub fn validate(&self) -> RlabResult<()> {
        if self.schema_version != 1 {
            return Err(RlabError::Validation {
                message: "unsupported external step schema_version".to_string(),
            });
        }
        if self.command.is_empty() || self.command.iter().any(|part| part.trim().is_empty()) {
            return Err(RlabError::Validation {
                message: "external step command cannot be empty".to_string(),
            });
        }
        if self.timeout_seconds == 0 {
            return Err(RlabError::Validation {
                message: "external step timeout_seconds must be greater than zero".to_string(),
            });
        }
        Ok(())
    }
