...
pub fn search_project(
    ...
    term: &str,
    kind: Option<&str>,
) -> RlabResult<Vec<SearchHit>> {
    let index_path = paths.cache.join("search_index.json");
    let documents = if index_path.exists() {
        let content =
            fs::read_to_string(&index_path).map_err(|error| RlabError::io(&index_path, error))?;
    ...
    let mut hits = documents
        .into_iter()
        .filter(|document| match kind {
            Some(expected) => document.kind == expected,
            None => true,
        })
        .filter_map(|document| {
            ...
            if score == 0 {
                ...
                Some(SearchHit {
                    ...
                    id: document.id,
                    score,
                    snippet: document.text.chars().take(240).collect(),
                })
            }
        })
        .collect::<Vec<_>>();
    hits.sort_by_key(|right| std::cmp::Reverse(right.score));
    Ok(hits)
}
