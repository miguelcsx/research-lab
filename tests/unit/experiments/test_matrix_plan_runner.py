...
from rlab.experiments.matrix import (
    Grid,
    Sample,
    choice,
    expand_matrix,
...
def test_experiment_plan_and_result_models() -> None:
    result = ExperimentResult(
        name="test",
        steps=(
            ExperimentStep(job_id="0001", params={}, metrics={"acc": 0.9}),
            ExperimentStep(job_id="0002", params={}, error="OOM"),
        ),
    )
...
def test_error_classification() -> None:
    ...
    assert (
        _classify_exception(OverflowError("overflow in computation"))
        == FailureKind.NUMERICAL_INSTABILITY
    )
...
def test_execute_experiment_with_benchmarks_only_skip_and_retry(runtime: RuntimeContext) -> None:
    ...
    def length_bench(target: object, ctx: object) -> dict[str, float]:
        return {"tokens": float(len(target.encode("test")))}  # type: ignore[attr-defined]
    ...
    register(
        runtime.registry, EntryKind.BENCHMARK, "test.length", length_bench, target_kind="tokenizer"
    )
    register(
        runtime.registry, EntryKind.BENCHMARK, "retry.bench", flaky_bench, target_kind="tokenizer"
    )

    exp = Experiment(
        question="length test",
        matrix={"target": ["tokenizer:project.byte"]},
        benchmarks=("test.length",),
    )
    assert (
        "test.length.tokens"
        in execute_experiment(runtime, build_plan("length", exp), exp).steps[0].metrics
    )
    assert (
        execute_experiment(runtime, build_plan("skip", exp), exp, skip=frozenset({"0000"})).steps
        == ()
    )
    ...
    assert (
        execute_experiment(runtime, build_plan("retry", retry_exp), retry_exp, partial=True)
        .steps[0]
        .error
        is None
    )
...
def test_execute_experiment_workflow_run_fn_and_evaluations(runtime: RuntimeContext) -> None:
    ...
    workflow_exp = Experiment(
        question="workflow execution test", matrix={}, workflow="test.workflow"
    )
    ...
    eval_exp = Experiment(
        question="eval test",
        matrix={"model": ["model:project.constant"]},
