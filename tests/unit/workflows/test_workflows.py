...
def test_workflow_models_are_immutable() -> None:
    ...
    assert ExternalStep(name="lean.build", command=("lake", "build"), timeout_seconds=300).timeout_seconds == 300
    with pytest.raises(Exception):
        workflow.steps = ("new_step",)  # type: ignore[misc]
...
def test_workflow_registry_lookup_and_invalid_step(runtime: RuntimeContext) -> None:
    ...
    assert run_workflow(Workflow(steps=("test.step",)), runtime).metric("registry_metric") is not None
...
def test_workflow_step_signature_variants(runtime: RuntimeContext) -> None:
    ...
    assert run_workflow(Workflow(steps=(WorkflowStep(name="two", fn=two_args),)), runtime).metric("two_arg_result") is not None
    assert run_workflow(Workflow(steps=(WorkflowStep(name="none", fn=no_args),)), runtime).metric("no_arg_result") is not None


def test_external_workflow_step_success_failure_timeout_and_parsers(runtime: RuntimeContext, tmp_path: Path) -> None:
    run_dir = tmp_path / "run_001"
    run_dir.mkdir()
    ctx = runtime.model_copy(update={"run_dir": run_dir})

    success = run_external_step(ExternalStep(name="echo_step", command=("echo", "hello_world")), ctx)
    ...
    parsed = run_external_step(
        ExternalStep(name="parsed_step", command=("echo", "result"), parser=lambda _stdout: {"parsed_score": 1.0}),
        ctx,
    )
    ...
    with pytest.raises(Exception):
        run_external_step(ExternalStep(name="slow", command=("sleep", "100"), timeout_seconds=0), ctx)


def test_parse_result_variants() -> None:
    bundle = ResultBundle(metrics=(Metric(name="score", value=0.9),))
    assert _parse_result(ExternalStep(name="b", command=("echo", "x"), parser=lambda _s: bundle), "out", None).metric("score") is not None
    assert _parse_result(ExternalStep(name="d", command=("echo", "x"), parser=lambda _s: {"accuracy": 0.8}), "out", None).metric("accuracy") is not None
