...
def test_notes_search_journal_and_ideas(project: Path) -> None:
    ...
    assert_success(invoke_cli(project, "journal", "decision", "add", "Promote model_v3 based on accuracy"))
    assert_success(invoke_cli(project, "journal", "decision", "list"), "Promote")
    assert_success(
        invoke_cli(project, "journal", "negative", "add", "dedup helps", "tried minhash", "no improvement")
    )
...
def test_baseline_graph_invalidation_and_impact(project: Path) -> None:
    ...
    assert_success(invoke_cli(project, "invalidate", "dataset:test", "--reason", "test contamination"))
...
def test_lint_ci_freeze_plan_exec_diff_and_handoff(project: Path) -> None:
    ...
    assert_success(invoke_cli(project, "plan", "power", "--effect-size", "0.1", "--variance", "1.0"))
    assert_success(invoke_cli(project, "plan", "cost", str(project / "experiments" / "000_smoke.py"), "--seconds-per-job", "60"))
    assert_success(invoke_cli(project, "exec", "--name", "echo_test", "echo", "42"))
    assert_failure(invoke_cli(project, "exec", "--name", "fail_cmd", "false"))
    assert_success(invoke_cli(project, "exec", "--name", "cmd_with_parser", "--parser", "rlab.nonexistent:parse", "echo", "42"))
